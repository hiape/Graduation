...
bool G711aEncoder::Encode(const short int* pcm_data_ptr, unsigned char* encoded_data_ptr)
{
	if (!encoded_data_ptr)
	{
...
bool G711uEncoder::Encode(const short int* pcm_data_ptr, unsigned char* encoded_data_ptr)
{
	if (!encoded_data_ptr)
	{
		std::cerr << __FILE__ << " " << __LINE__ << "encoded_data_ptr is null" << std::endl;
